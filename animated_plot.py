...
def update_node_status(iteration):
    ...
    nodes = nx.draw_networkx_nodes(graph, pos=pos, node_color=node_colors, node_size = NODE_SIZE)
...
if __name__ == "__main__":
    ...
    if EDGE_LABELS == True:
        edge_labels = {(u, v): f'{d["ownership"]:.2f}' for u, v, d in graph.edges(data=True)}
        print(edge_labels)
        edge_label = nx.draw_networkx_edge_labels(graph, pos = pos, edge_labels=edge_labels, font_size = 8)

    if ANIMATION == True:
        animation = FuncAnimation(fig, update_node_status, frames=10, interval=1000, blit = False)
